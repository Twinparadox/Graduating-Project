...
file_list = ['SAMSUNG_Electronics.csv', 'NAVER.csv', 'HYUNDAI_Motors.csv', 'POSCO.csv', 'Kakao.csv',
             ...
             'SAMSUNG_Life.csv', 'Hyundai_Mobis.csv', 'KEPCO.csv']

length = len(file_list)
for idx in range(length):
    stock = data.get_data_yahoo(stock_list[idx],start=start_date,end=end_date,interval="1d")
...
#     stock = pd.DataFrame()
#     e_date = end_date
#     s_date = datetime.strptime(e_date, '%Y-%m-%d') - timedelta(days=59)
#     s_date = s_date.strftime('%Y-%m-%d')
#     stock = data.get_data_yahoo(stock_list[idx],start=s_date,end=e_date,interval="5m")
#
#     stock.to_csv(path+e_date+'/'+file_list[idx],mode='w')