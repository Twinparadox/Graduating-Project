...
router.get('/', function(req, res, next) {
    ...
    var sql = "SELECT trading_price FROM trading_logs WHERE trading_corp='"+corp+"' ORDER BY trading_id DESC LIMIT " + days;
...
router.post('/', function(req, res, next) {
    ...
    var sql = "SELECT * FROM trading_logs WHERE trading_corp='"+corp+"' ORDER BY trading_id DESC LIMIT " + days;
    connection.query(sql, function(error, rows, fields) {
        if(!error) {
            var result = {corp:corp, trading_date:[], trading_price:[], trading_volume:[], trading_action:[]};
